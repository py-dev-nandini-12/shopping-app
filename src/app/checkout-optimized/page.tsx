...
import React, { useState, useActionState, useOptimistic, startTransition } from "react";
...
import { validateShippingAction, validatePaymentAction } from "@/app/checkout-actions";
...
export default function CheckoutPage() {
  ...
  const [shippingState, shippingAction, isShippingPending] = useActionState(validateShippingAction, {
    success: false,
    step: 1,
  });

  // React 19 useActionState for payment validation
  const [paymentState, paymentAction, isPaymentPending] = useActionState(validatePaymentAction, {
    success: false,
    step: 2,
  });

  // React 19 useOptimistic for order completion
  const [orderComplete, setOrderComplete] = useOptimistic(false, (current, newValue: boolean) => newValue);
  ...
  }, [paymentState.success, paymentState.orderId, cartState, clearCartOptimistic, setOrderComplete]);
                  ...
                  <form action={shippingAction} className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <input
                        ...
                        name="city"
                        placeholder="City"
                        className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-rose-500 focus:border-rose-500 transition-all duration-300 bg-white text-gray-900 placeholder-gray-500"
                        required
                      />
                      <input
                        type="text"
                    ...
                    <div className="flex justify-between pt-4">
                      ...
                      >
                        {isShippingPending ? "Validating..." : "Continue to Payment"}
                      </Button>
                    </div>
                  </form>
                </CardContent>
                ...
                <CardContent className="p-12 text-center">
                  ...
                  {paymentState.orderId && (
                    <p className="text-sm text-gray-500 mb-8">
                      Order ID: <span className="font-mono font-semibold">{paymentState.orderId}</span>
                    </p>
                  )}
                ...
                <div className="space-y-4">
                  ...
                  <div className="border-t pt-4">
                    ...
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <span className="font-semibold">${(cartState.total * 0.08).toFixed(2)}</span>
                    </div>
